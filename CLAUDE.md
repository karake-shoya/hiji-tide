...
- 🔴 **潮汐の推算ロジック（`js/tide.js` / `js/astro.js` / `js/fishing.js`）は動作の根幹。触ったら `node --test` を通す。**
  `test/fixtures/tide-2026.json` がリファクタ前の推算結果（2026年の全日・1時間刻み）。1点でもずれたら計算が変わっている。
- ⚠ **`css/` か `js/` に「ファイルを足したら」`sw.js` の `SHELL` に書き足し `VERSION` を上げる。**
  中身を変えただけなら不要（`css` と `js` はネットワーク優先で配るので、次に開けば最新が出る）。
